...
public class NavMeshNoGrid : MonoBehaviour {
	...
	void Start () {
        ...
        foreach (TreeInstance tree in tData.treeInstances){
            ...
            //center
            if (isTreeInRange(tree.position, tX, tY)) hMap[tX,tY] = float.MaxValue;

            for (int i = 1; i <= propApproximationRadius; i++){
                //top 3
                if (isTreeInRange(tree.position, tX - i, tY + i)) hMap[tX - i, tY + i] = float.MaxValue;
                if (isTreeInRange(tree.position, tX, tY + i)) hMap[tX, tY + i] = float.MaxValue;
                if (isTreeInRange(tree.position, tX + i, tY + i)) hMap[tX + i, tY + i] = float.MaxValue;
                //left & right from center
                if (isTreeInRange(tree.position, tX - i, tY)) hMap[tX - i, tY] = float.MaxValue;
                if (isTreeInRange(tree.position, tX + i, tY)) hMap[tX + i, tY] = float.MaxValue;
                //bottom 3
                if (isTreeInRange(tree.position, tX - i, tY - i)) hMap[tX - i, tY - i] = float.MaxValue;
                if (isTreeInRange(tree.position, tX, tY - i)) hMap[tX, tY - i] = float.MaxValue;
    ...
    private List<Node> getNeighbors(Node cur, Vector3 goal){
        ...
        if(cur.parent != null){
            ...
            //diagonal case
            if(dx != 0 && dy != 0){
                if (isReachable(cur.x,cur.y,cur.x, cur.y + dy)){
                    nb = new Node(cur, cur.x, cur.y + dy, getWorldPos(cur.x, cur.y + dy), goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (isReachable(cur.x, cur.y, cur.x + dx, cur.y)){
                    nb = new Node(cur, cur.x + dx, cur.y, getWorldPos(cur.x + dx, cur.y), goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (neighbors.Count > 0 && isReachable(cur.x, cur.y, cur.x + dx, cur.y + dy)){
                    nb = new Node(cur, cur.x + dx, cur.y + dy, getWorldPos(cur.x + dx, cur.y + dy), goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (isReachable(cur.x, cur.y, cur.x - dx, cur.y) == false && isReachable(cur.x, cur.y, cur.x, cur.y + dy) && isReachable(cur.x, cur.y, cur.x - dx, cur.y - dy)){
                    nb = new Node(cur, cur.x - dx, cur.y - dy, getWorldPos(cur.x - dx, cur.y - dy), goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (isReachable(cur.x, cur.y, cur.x, cur.y - dy) == false && isReachable(cur.x, cur.y, cur.x + dx, cur.y) && isReachable(cur.x, cur.y, cur.x + dx, cur.y - dy)){
                    nb = new Node(cur, cur.x + dx, cur.y - dy, getWorldPos(cur.x + dx, cur.y - dy), goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                ...
                if (isReachable(cur.x, cur.y, cur.x, cur.y + dy))
                {
                    ...
                    if (isReachable(cur.x, cur.y, cur.x + 1, cur.y) == false && isReachable(cur.x, cur.y, cur.x + 1, cur.y + dy)){
                ...
                if (isReachable(cur.x, cur.y, cur.x + dx, cur.y)){
                    ...
                    if (isReachable(cur.x, cur.y, cur.x, cur.y + 1) == false && isReachable(cur.x, cur.y, cur.x + dx, cur.y + 1)){
            ...
            if (isReachable(cur.x, cur.y, cur.x - 1, cur.y + 1)){
                nb = new Node(cur, cur.x - 1, cur.y + 1, getWorldPos(cur.x - 1, cur.y + 1), goal);
                if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
            }
            if (isReachable(cur.x, cur.y, cur.x, cur.y + 1)){
            ...
            //center 2
            if (isReachable(cur.x, cur.y, cur.x + 1, cur.y)){
    ...
    private Coordinate Jump(int x, int y, int px, int py, Vector3 goal){
        ...
        if(isReachable(px,py,x,y) == false){
            return null;
        ...
        if(dx != 0 && dy != 0){
            if ((isReachable(px, py, x - dx, y + dy) && isReachable(px, py, x - dx, y) == false) ||
               (isReachable(px, py, x + dx, y - dy) && isReachable(px, py, x, y - dy) == false)) {
                    return new Coordinate(x,y);
            }
        }else{//horizontal/vertical
            if(dx != 0){
                if ((isReachable(px, py, x + dx, y + 1) && isReachable(px, py, x, y + 1) == false) ||
                   (isReachable(px, py, x + dx, y - 1) && isReachable(px, py, x, y - 1) == false)){
                    return new Coordinate(x,y);
                }
            }else{
                if ((isReachable(px, py, x + 1, y + dy) && isReachable(px, py, x + 1, y) == false) ||
                   (isReachable(px, py, x - 1, y + dy) && isReachable(px, py, x - 1, y) == false)){
                        return new Coordinate(x,y);
                   }
            }
        }
        ...
        if (isReachable(px, py, x + dx, y) || isReachable(px, py, x, y + dy)){
    ...
    private bool isReachable(int px, int py, int x, int y){
       if(x > -1 && y > -1 && x < w && y < h) return (Mathf.Abs(hMap[px,py] - hMap[x,y]) < maxHeightDelta);
       return false;
    }

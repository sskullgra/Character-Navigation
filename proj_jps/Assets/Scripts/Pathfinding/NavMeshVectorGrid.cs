...
public class NavMeshVectorGrid : MonoBehaviour {
    public GameObject gizmo;

    public Material normal,bad,path_material,startend;
	...
	void Start () {
        ...
        foreach (TreeInstance tree in tData.treeInstances){
            ...
            //center
            if (isTreeInRange(tree.position, tX, tY)) grid[tX,tY] = null;

            for (int i = 1; i <= propApproximationRadius; i++){
                //top 3
                if (isTreeInRange(tree.position, tX - i, tY + i)) grid[tX - i, tY + i] = null;
                if (isTreeInRange(tree.position, tX, tY + i)) grid[tX, tY + i] = null;
                if (isTreeInRange(tree.position, tX + i, tY + i)) grid[tX + i, tY + i] = null;
                //left & right from center
                if (isTreeInRange(tree.position, tX - i, tY)) grid[tX - i, tY] = null;
                if (isTreeInRange(tree.position, tX + i, tY)) grid[tX + i, tY] = null;
                //bottom 3
                if (isTreeInRange(tree.position, tX - i, tY - i)) grid[tX - i, tY - i] = null;
                if (isTreeInRange(tree.position, tX, tY - i)) grid[tX, tY - i] = null;
    ...
    void PathfindingWindow(int id){
        ...
        Vector3? start = grid[sx, sy];
        Vector3? goal = grid[gx, gy];

        GUILayout.BeginHorizontal();
        if (start != null && goal != null && GUILayout.Button("Find Path"))
        {            
    ...
    private void map(int x, int y) {
        grid[x, y] = new Vector3(meshScale.x * x, meshScale.y * hMap[x, y], meshScale.z * y) + tPos; ;
    ...
    private List<Node> getAllNeighbors(Node cur, Vector3? goal){
        ...
        Node nb = null; Vector3? pos = null;
        //top 3
        if (cur.y + 1 < h)
        {
            if (cur.x - 1 > -1)
            {
                if ((pos = (grid[cur.x - 1, cur.y + 1])) != null){
                    nb = new Node(cur, cur.x - 1, cur.y + 1, pos, goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }                
            }

            if ((pos = (grid[cur.x, cur.y + 1])) != null)
            {
                nb = new Node(cur, cur.x, cur.y + 1, pos, goal);
                if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
            }            
        ...
        if (cur.x + 1 < w)
        {
            if ((pos = (grid[cur.x + 1, cur.y])) != null)
            {
                nb = new Node(cur, cur.x + 1, cur.y, pos, goal);
                if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
            }            
        }
        ...
        if(cur.parent != null){
            ...
            if(dx != 0 && dy != 0){
                if (isReachable(cur.x, cur.y + dy)) {
                    nb = new Node(cur, cur.x, cur.y + dy, grid[cur.x, cur.y + dy], goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (isReachable(cur.x + dx, cur.y)) {
                    nb = new Node(cur, cur.x + dx, cur.y, grid[cur.x + dx, cur.y], goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                if (neighbors.Count > 0 && isReachable(cur.x + dx, cur.y + dy)) {
                    nb = new Node(cur, cur.x + dx, cur.y + dy, grid[cur.x + dx, cur.y + dy], goal);
                    if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                }
                ...
                if (isReachable(cur.x, cur.y - dy) == false && isReachable(cur.x + dx, cur.y) && isReachable(cur.x + dx, cur.y - dy)) {
             	...
             	if (isReachable(cur.x, cur.y + dy)){
                    ...
                    if (isReachable(cur.x + 1, cur.y) == false && isReachable(cur.x + 1, cur.y + dy)){
                        nb = new Node(cur, cur.x + 1, cur.y + dy,grid[cur.x + 1, cur.y + dy], goal);
                        if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                    }
                    if (isReachable(cur.x - 1, cur.y) == false && isReachable(cur.x - 1, cur.y + dy)) {
                        nb = new Node(cur, cur.x - 1, cur.y + dy, grid[cur.x - 1, cur.y + dy], goal);
                ...
                if (isReachable(cur.x + dx, cur.y)){
                    ...
                    if (isReachable(cur.x, cur.y + 1) == false && isReachable(cur.x + dx, cur.y + 1)) {
                        nb = new Node(cur, cur.x + dx, cur.y + 1, grid[cur.x + dx, cur.y + 1], goal);
                        if (nb.Equals(cur.parent) == false) neighbors.Add(nb);
                    }
                    if (isReachable(cur.x, cur.y - 1) == false && isReachable(cur.x + dx, cur.y - 1)) {
                        nb = new Node(cur, cur.x + dx, cur.y - 1, grid[cur.x + dx, cur.y - 1], goal);
    ...
    private bool isReachable(int x, int y){
        return (x > -1 && y > -1 && x < w && y < h) ? (grid[x,y] != null)  : false;
    }
    ...
    private IEnumerator findPath(Vector3? start, Vector3? goal)//
    {
        ...
        Coordinate sco = getGridPos(start.Value);
        if(sco == null || grid[sco.x,sco.y] == null) yield break; //return path;

        Coordinate gco = getGridPos(goal.Value);
        if (gco == null || grid[gco.x, gco.y] == null) yield break;//return path;

        if(sco.x == gco.x && sco.y == gco.y) yield break;//return path;

        Vector3? correctedGoal = grid[gco.x,gco.y];
        Vector3? correctedStart = grid[sco.x,sco.y];
        ...
        while(openList.Count > 0){
            ...
            foreach(Node neighbor in (cur.neighbors == null) ? (cur.neighbors = getNeighbors(cur, correctedGoal)) : cur.neighbors){
                ...
                jn = new Node(cur,jmp.x,jmp.y,grid[jmp.x,jmp.y],goal);
        ...
        //}
        gizmo.renderer.material = path_material;
